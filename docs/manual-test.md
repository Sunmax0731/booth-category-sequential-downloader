...
- 作業ディレクトリ: `D:\AI\ChromeExtension\booth-category-sequential-downloader`
...
```powershell
cd D:\AI\ChromeExtension\booth-category-sequential-downloader
npm test
npm start
```
...
5. Chrome拡張 Side Panel + downloads API の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/booth-category-sequential-downloader-docs.zip` に存在する。
